    ...
    <div class="h-100 overflow-auto" style="height: 60vh !important">
        <table class="table table-hover table-striped bg-light" id="plist">
            ...
            <tbody>
                ...
                $qry = $conn->query($sql);
                $totalAvailableQuantityValue = 0; // Variable to store the total available quantity value
                while($row = $qry->fetch_assoc()):
                    ...
                    $qty = $qty > 0 ? $qty : 0;
                    $quantityValue = $row['price'] * $qty;
                    $totalAvailableQuantityValue += $quantityValue; // Add the quantity value to the total
                ?>
                <tr class="item <?php echo $qty < 50 ? "bg-danger bg-opacity-25" : '' ?>" data-id="<?php echo $row['product_id'] ?>">
                    ...
                    <td class="td py-0 px-1 text-center qty-value"><?php echo format_num($quantityValue) ?></td>
                </tr>
                <?php endwhile; ?>

                <!-- Add a row for Total Available Quantity Value -->
                <tr>
                    <td colspan="5" class="text-end fw-bold">Total Available Quantity Value:</td>
                    <td class="fw-bold text-center"><?php echo format_num($totalAvailableQuantityValue) ?></td>
                </tr>
            </tbody>
        </table>
    </div>
