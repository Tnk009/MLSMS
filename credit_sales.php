...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $price = -$_POST['price']; // Convert the price to a negative value for credit sales

    // Perform the necessary calculations and database operations here
    // ...

    // Insert the credit sale data into the sales_report table
    $sql = "INSERT INTO sales_report (product_id, quantity, price) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iii", $product_id, $quantity, $price);
    $stmt->execute();

    // Redirect to the variance_report.php page after the credit sale is recorded
    header("Location: variance_report.php");
    exit();
}

// Fetch the product data from the database
$sql = "SELECT product_id, name FROM product_list WHERE delete_flag = 0";
...
<table>
    ...
    <thead>
        <tr>
            ...
            <th class="py-0 px-1">Available Quantity Value</th>
        </tr>
    </thead>
        ...
        <table class="table table-hover table-striped bg-light" id="plist">
            ...
            <tbody>
                ...
                <tr class="item <?php echo $qty < 50 ? "bg-danger bg-opacity-25" : '' ?>" data-id="<?php echo $row['product_id'] ?>">
                    ...
                    <td class="td py-0 px-1 text-center price"><?php echo format_num($row['price']) ?></td>
                    <td class="td py-0 px-1 text-center qty"><?php echo $qty ?></td>
                    <td class="td py-0 px-1 text-center qty-value"><?php echo $row['price'] * $qty ?></td>
                </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
                                ...
                                <div class="h-100 d-flex w-100 flex-column">
                                    ...
                                    <div class="pt-2">
                                        ...
                                        <div class="w-100 mx-0 d-flex pb-1 align-items-center">
                                            <div class="col-4 pe-2 fs-6 fw-bolder text-light">Tax (16.5%)<small>Inclusive</small></div>
...
<script>
  ...
  var payableAmount = -sub; // Make the payable amount negative

  $('[name="total"]').val(parseFloat(payableAmount));
  $('#total').text(parseFloat(payableAmount).toLocaleString('en-US', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }));
  $('#subTotal').text(parseFloat(payableAmount).toLocaleString('en-US', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }));
  discount = payableAmount * 0.165;
  $('#tax').text(parseFloat(discount).toLocaleString('en-US', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }));
}
    ...
    $('#plist tbody tr').click(function() {
        ...
        var qtyValue = parseFloat(price) * parseFloat(qty);
        ntr.append('<td class="py-0 px-1 align-middle text-end qty-value">' + qtyValue.toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2 }) + '</td>');
