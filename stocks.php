        ...
        <table class="table table-hover table-striped table-bordered">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center p-0">Value</th>
                    <th class="text-center p-0">Action</th>
                </tr>
            </thead>
            <tbody>
                <?php
                $sql = "SELECT s.*, p.name as pname, p.product_code, p.price FROM `stock_list` s INNER JOIN `product_list` p ON s.product_id = p.product_id WHERE p.delete_flag = 0 ORDER BY UNIX_TIMESTAMP(s.date_added) DESC";
                $qry = $conn->query($sql);
                $i = 1;
                ...
                ?>
                    <tr class="<?php echo strtotime(date("Y-m-d")) > strtotime($row['expiry_date']) ? 'bg-danger bg-opacity-50' : '' ?>">
                        ...
                        <td class="py-0 px-1 text-end"><?php echo format_num($row['quantity'] * $row['price']) ?></td>
...
<script>
    function addStock(quantity, price) {
        var productValue = quantity * price;
        var totalStockValue = parseFloat($('#total-stock-value').text().replace(/,/g, ''));
        totalStockValue += productValue;
        $('#total-stock-value').text(format_num(totalStockValue));
    }

    function deleteStock(quantity, price) {
        var productValue = quantity * price;
        var totalStockValue = parseFloat($('#total-stock-value').text().replace(/,/g, ''));
        totalStockValue -= productValue;
        $('#total-stock-value').text(format_num(totalStockValue));
    ...
    function delete_data($id){
        ...
        // Call deleteStock() function to update the total stock value
        var quantity = parseInt($(this).closest('tr').find('td:nth-child(4)').text().replace(/,/g, ''));
        var price = parseFloat($(this).closest('tr').find('td:nth-child(5)').text().replace(/,/g, ''));
        deleteStock(quantity, price);
    }
</script>
