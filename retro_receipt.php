...
// Retrieve the description from the query parameter
...
<div class="container-fluid">
    <div id="outprint_receipt">
        <div class="text-center fw-bold lh-1">
            <span>Mthira Liquors Sales System</span><br>
            <small>Retrospective Sale Receipt</small>
        </div>
        ...
        <div class="fs-6 fs-light d-flex w-100 mb-1">
            <span class="col-auto pe-2">Description:</span> 
            <span class="border-bottom border-dark flex-grow-1"><?php echo $description ?></span>
        </div>
        <table class="table table-striped">
            ...
            <tbody>
            <?php
            $items = $conn->query("SELECT i.*, p.name as pname, p.product_code, i.description FROM `transaction_items` i INNER JOIN product_list p ON i.product_id = p.product_id WHERE `transaction_id` = '{$transaction_id}'");

            while ($row = $items->fetch_assoc()):
                $description = isset($_POST['description']) ? $_POST['description'] : ''; // Initialize description variable
            ?>
            <tr>
                        ...
                        <td class="px-1 py-0 align-middle">
                            ...
                            <div class="fw light lh-1">
                                ...
                                <small><?php echo htmlspecialchars($row['description']); ?></small>                            </div>
                        </td>
            ...
            <tfoot>
                ...
                <tr>
                    <th class="px-1 py-0" colspan="2">Tax 16.5%</th>
                    <th class="px-1 py-0 text-end"><?php echo format_num($total * .165) ?></th>
                </tr>
                <tr>
                ...
                <tr>
                    <th class="px-1 py-0" colspan="2">Description</th>
                    <th class="px-1 py-0 text-end"><span id="description_value"><?php echo htmlspecialchars($description); ?></span></th>                
                </tr>
            </tfoot>
        </table>
    </div>
