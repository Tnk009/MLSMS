...
<div class="card rounded-0 shadow">
    ...
    $qry = $conn->query($sql);
    $i = 1;
    $subtotal_positive = 0; // Initialize the positive subtotal variable
    $subtotal_negative = 0; // Initialize the negative subtotal variable
    while($row = $qry->fetch_assoc()):
        ...
        if ($row['total'] > 0) { // Consider only positive sales
            $subtotal_positive += $row['total']; // Add the total to the positive subtotal
        }
        if ($row['total'] < 0) { // Consider only negative sales
            $subtotal_negative += $row['total']; // Add the total to the negative subtotal
        }
    ?>
    <tr>
    ...
    <tr>
        ...
        <td class="text-end"><?php echo format_num($subtotal_positive + ($subtotal_negative * -1)) ?></td>
        <td></td>
    </tr>
    <tr>
        <td colspan="4" class="text-end fw-bold">Total Cash Sales</td>
        <td class="text-end"><?php echo format_num($subtotal_positive) ?></td>
        <td></td>
    </tr>
    <tr>
        <td colspan="4" class="text-end fw-bold">Variance</td>
        <td class="text-end"><?php echo format_num($subtotal_negative) ?></td>
        <td></td>
    </tr>
...
<script>
    $(function(){
        ...
        $('#print').click(function(){
            ...
            el.append("<div class='text-center lh-1 fw-bold'>Mthira Liquor's Sales Report<br/>As of<br/>"+date_range+"</div><hr/>")
